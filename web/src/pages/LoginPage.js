...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Save session in ALL possible localStorage formats Supabase SDK checks
      const session = {
      ...
      // Supabase v2 uses this exact key format
      const storageKey = `sb-tonlofhigkpbcsmfesjq-auth-token`;
      localStorage.setItem(storageKey, JSON.stringify(session));

      // Also save role so dashboard can read it without hitting DB again
      localStorage.setItem('vx_role', profile?.role || 'admin');
      localStorage.setItem('vx_user_id', data.user.id);
      localStorage.setItem('vx_email', data.user.email);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060B14', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ background: '#111D2E', border: '1px solid rgba(0,212,255,0.25)', borderRadius: '12px', padding: '2rem' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '1.25rem' }}>
            <a href="/customer/lookup" style={{ color: '#3D5872', fontSize: '0.78rem', textDecoration: 'none' }}>Customer? Check loan status →</a>
          </div>
        </div>
      </div>
